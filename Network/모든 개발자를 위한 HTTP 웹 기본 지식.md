...
- 캐시 유효 시간을 초과하면, 서버를 통해 데이터를 다시 조회하고, 캐시를 갱신함
- 다시 네트워크 다운로드가 발생함
...
- 캐시 유효 시간이 초과해서 서버에 다시 요청하면 다음 두 가지 상황이 나타남
    - 서버에서 기존 데이터를 변경함
    - 서버에서 기존 데이터를 변경하지 않음 ⇒ 클라이언트의 데이터와 서버의 데이터가 같다는 것을 확인하면 재사용 사능
...
- 캐시 유효 시간이 초과해도, 서버의 데이터가 갱신되지 않을 경우 304 Not Modified + 헤더 메타 정보만 응답(바디X)
...
- 서버에서 별도의 캐시 로직을 관리하고 싶은 경우를 다룰 수 없음
...
- 로딩 시간이 빨라짐
- EX) 외국 유튜브 컨텐츠를 한국에서 볼 경우, 본 사람이 별로 없다면 느리다.
    - 프록시 캐시 서버에 데이터가 없기 때문에 다운로드 받음
...
- 통장 잔고나 카드 비밀번호처럼 캐시가 필요 없는 경우 캐시 무효화가 필요함

### 확실한 캐시 무효화 응답

- 과거 브라우저까지 지원을 해야 되기 때문에 아래 지시어를 다 넣자
